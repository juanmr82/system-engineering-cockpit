...
public fun Route.moduleRoutes(
    ...
    route(ApiPaths.MODULES) {
        ...
        /**
         * Module configuration — the system level and the mandatory-attribute set.
         *
         * `sec-admin`, not `sec-user`, and the split inside Tier 2 is deliberate rather than
         * incidental (spec §3): a comment is one reviewer's note on one object and belongs to
         * anyone who can see it, while a mandatory-attribute policy governs every object in the
         * module and changes what the Issues column says about all of them. Both are `:__Meta`;
         * only one is an opinion.
         *
         * Visibility is checked underneath this, not instead of it — `MetaWriter` resolves the
         * module through the filtered `moduleExists`, so an administrator still cannot configure a
         * module their groups cannot read (§3: the two axes are independent).
         */
