...
The exact Cypher is settled by measurement in phase 2, not by preference. Two candidate forms, both
correct:

```cypher
-- A: property comparison per candidate node
WHERE $seesAll OR EXISTS {
  (n)-[:__inAccessCategory]->(c:__AccessCategory) WHERE c.__metaId IN $acl
}

-- B: categories pinned once, then a pure degree check
...
**B is expected to win** — it reads no properties in the inner loop and the category nodes are
already in the page cache — but B costs a `WITH` that must be threaded through every query, and it
is only worth it if the difference is real. `PROFILE` both against the reference module (984
objects) and the JIRA set (784 issues), **put the db-hit numbers in ADR 0016**, and then use one
form everywhere. Mixed forms are how a filter gets forgotten.
